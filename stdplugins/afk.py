...
async def on_afk(event):
    ...
    if borg.storage.USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"I'm afk since {afk_since = f"`{int(hours)}h{int(minutes)}m` **ago**"} " + \
            f"and I will be back soon\n__Reason:__ {reason}" \
            if reason \
            else f"I'm afk since {afk_since = f"`{int(hours)}h{int(minutes)}m` **ago**"} and I will be back soon."
